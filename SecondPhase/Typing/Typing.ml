...
let rec searchClass (name:Type.ref_type) (scope:AST.astclass list) =
	match scope with 
	| [] -> raise (Invalid_inheritance ("Class: "^(flatlistDot name.tpath)^"."^name.tid^" not found"))
	| elem::rest -> (
			match name.tpath with 
			| [] -> 
				if elem.id=name.tid then elem
				else searchClass name rest
			| first::others -> 
				if elem.id=first then 
					let c,i = colectClassInfo elem.ctypes in
					searchClass {tpath=others; tid=name.tid} c
				else searchClass name rest
		)
...
let rec verifyClassDependency (cl:AST.astclass) (classesScope:AST.astclass list) (chain:string list)=
	if inlist cl.id chain then
		raise (Recursive_inheritance ("Class: "^cl.id^" inherits from itself"))
			...
			else verifyClassDependency (searchClass cl.cparent classesScope) classesScope (cl.id::chain)
